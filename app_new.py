...
def parse(ticker):
    ...
    def get_row(df, name):
        try:
            return df.loc[name].values.tolist()
        except:
            return []
    ...
    op_fcfs = get_row(cf, "Operating Cash Flow")[:5]
    capexs = get_row(cf, "Capital Expenditure")[:5]
    net_income = get_row(cf, "Net Income From Continuing Operations")[:5]

    try:
        min_len = min(len(op_fcfs), len(capexs))

        if min_len == 0:
            fcfs_equity = []
        else:
            fcfs_equity = [
                float(op_fcfs[i]) + float(capexs[i])
    ...
    revenues = get_row(inc, "Total Revenue")
    ...
    # -------------------------
    try:
        ge = stock.info.get("earningsQuarterlyGrowth", None)
        if ge is None:
            ge = []
        else:
            ge = float(ge) * 100
...
def dcf(data):
    ...
    min_len = min(len(data['ni']), len(data['revenues']))

    ni_margins = (
    np.round(
        np.array(data['ni'][:min_len]) /
        np.array(data['revenues'][:min_len]),
        2
    ) * 100
)

    forecast = [data['fcf'][1], data['fcf'][0]]
    rev_forecast_df = [data['revenues'][1], data['revenues'][0]]
    ...
    print("Revenue Forecast:", rev_forecast_df)
